...
function solve_decision_model(model::JuMP.Model,
    ...
    if isa(wildtype, Nothing) || isa(percent_suppression,Nothing) || isa(target_timestep,Nothing) #|| isa(which_node, Nothing)
        ...
        # Expression
        JuMP.@objective(model, Min, (sum((F[n,o,s,wildtype,t] - percent_suppression*F[n,o,s,wildtype,1]).^2
        for n in N, o in O, s in SF, t in T[target_timestep:end]) + 1e-8*sum(control_M)))
...
function solve_decision_model_wolb(model::JuMP.Model,
    objective_function::Type{<:TargetPercentageByDate};
    wildtype=nothing,
    ...
    # Expression
    JuMP.@objective(model, Min, (sum((F[n,o,s,wildtype,t] - percent_suppression*F[n,o,s,wildtype,1]).^2
    for n in N, o in O, s in SF, t in T[target_timestep:end])
        + 1e-8*sum(control_M)
        + 1e-8*sum(control_F))
        )
...
function solve_decision_model(model::JuMP.Model,
    ...
    JuMP.@objective(
        model, Min, sum((F[n,o,s,wildtype,t] - F[n,o,s,wildtype,1]*(percent_to_suppress)^t).^2
        for n in N[node_to_suppress], o in O, s in SF, t in T)
