...
sol_targperc = solve_decision_model_wolb(
    prob,
    TargetPercentageByDate;
    wildtype=wild_gene,
...
# SpatialReduction:
sol_spatial = solve_decision_model_wolb(
    prob,
    SpatialReduction;
    wildtype=wild_gene,
    node_to_suppress=4,
...
i_mip = JuMP.optimizer_with_attributes(
    ...
    "mip_solver" =>
        JuMP.optimizer_with_attributes(Gurobi.Optimizer, "NumericFocus" => 1),
)
...
# SpatialReduction:
sol_spatial_mip = solve_decision_model_wolb(
    prob_mip,
    SpatialReduction;
    wildtype=wild_gene,
    node_to_suppress=4,
